...
Reads the final game.js and index.html, then creates 34 incremental
commits by progressively appending sections.
"""
import subprocess, os, sys, textwrap

DIR = r"c:\Users\suraj\OneDrive\Desktop\Flappy Fahh"

def git(*args):
    result = subprocess.run(["git"] + list(args), cwd=DIR, capture_output=True, text=True)
...
# ── Read final versions ────────────────────────────────────────────────
with open(os.path.join(DIR, "game.js"), encoding="utf-8") as f:
    FINAL_GAME_JS = f.read()

with open(os.path.join(DIR, "index.html"), encoding="utf-8") as f:
    FINAL_HTML = f.read()

# ── Reset to original for incremental commits ─────────────────────────
print("Resetting to original files...")
git("checkout", "HEAD", "--", "game.js", "index.html")

# ── Define 34 commits as (section_name, game_js_snippet, html_snippet) ─
# Each commit adds a new block to game.js (and updates index.html once at the end)
# We'll accumulate game.js content block by block.

BLOCKS = [
    ("feat: ultra edition — project setup with aurora background and subtitle",
     '// =========================================================\n//  FLAPPY FAHH — game.js  ULTRA UPGRADE v3\n// =========================================================\n"use strict";\n',
     None),  # html updated later

    ("feat: DOM references for all new HUD elements",
     '''
// ── DOM references ─────────────────────────────────────────
...
''', None),
    ...
    ("feat: audio context and FAHH sound player with pitch rate control",
     '''
// ── Audio core ─────────────────────────────────────────────
...
''', None),
    ...
    ("feat: score and death sound effects using Web Audio API oscillators",
     '''
function playScore() {
  try {
...
''', None),
    ...
    ("feat: power-up ascending arpeggio jingle and coin ping sound",
     '''
...
''', None),
    ...
    ("feat: background chiptune music loop — triangle oscillator arpeggio",
     '''
...
''', None),
    ...
    ("feat: game constants — lives, power-up duration, double-flap timing",
     '''
// ── Constants ──────────────────────────────────────────────
...
''', None),
    ...
    ("feat: achievements system — 7 badges with localStorage and toast notification",
     '''
// ── Achievements ───────────────────────────────────────────
...
''', None),
    ...
    ("feat: local leaderboard — top 5 scores with medals, persisted to localStorage",
     '''
// ── Leaderboard ─────────────────────────────────────────────
...
''', None),
    ...
    ("feat: game state variables — lives, power-ups, combo, parallax, shake",
     '''
// ── Game State ─────────────────────────────────────────────
let state = "start";
...
''', None),
    ...
    ("feat: day/night sky cycle — 4 phases tied to score using color interpolation",
     '''
// ── Day/Night Cycle ────────────────────────────────────────
...
''', None),
    ...
    ("feat: HUD helpers — hearts, power-up bar, coins, combo, score, gravity, shake",
     '''
// ── HUD Helpers ────────────────────────────────────────────
...
''', None),
    ...
    ("feat: resetGame — full reset of all state including lives, power-ups, parallax",
     '''
// ── Reset ──────────────────────────────────────────────────
function resetGame() {
...
''', None),
    ...
    ("feat: startGame — unlocks first-flight achievement and starts background music",
     '''
function startGame() {
  if (audioCtx.state === "suspended") audioCtx.resume();
...
''', None),
    ...
    ("feat: showStartScreen — renders leaderboard and stops music",
     '''
...
''', None),
    ...
    ("feat: showDeadScreen — high score prompt, achievement checks, share button",
     '''
...
''', None),
    ...
    ("feat: handleFlap — double flap mechanic with pitch-scaled FAHH sound",
     '''
// ── Input ──────────────────────────────────────────────────
function handleFlap() {
...
''', None),
    ...
    ("feat: gravity toggle — tracks flip count for achievement, event listeners",
     '''
...
''', None),
    ...
    ("feat: pipe spawning — moving pipes that oscillate vertically above score 15",
     '''
// ── Spawning ───────────────────────────────────────────────
...
''', None),
    ...
    ("feat: particle effects — flap trail, gravity burst, score confetti, power-up nova",
     '''
// ── Particles ──────────────────────────────────────────────
...
''', None),
    ...
    ("feat: collision detection — circle-rect for pipes, circle-circle for pickups",
     '''
// ── Collision ──────────────────────────────────────────────
...
''', None),
    ...
    ("feat: lives system — 3 hearts, shield absorbs 1 hit, invincibility flash",
     '''
// ── Lives System ───────────────────────────────────────────
...
''', None),
    ...
    ("feat: physics update — gravity, double-flap recharge, magnet steering",
     '''
// ── Update ─────────────────────────────────────────────────
function update() {
  if (state !== "playing") return;
...
''', None),
    ...
    ("feat: power-up timer countdown and pipe scroll in update loop",
...
''', None),
    ...
    ("feat: moving pipes, scoring, milestone popup, achievement checks in update",
...
''', None),
    ...
    ("feat: coin and power-up collection logic in update loop",
...
''', None),
    ...
    ("feat: draw background — day/night gradient sky with twinkling parallax stars",
     '''
// ── Draw ───────────────────────────────────────────────────
...
''', None),
    ...
    ("feat: parallax cloud and hill layers scrolling at different speeds",
     '''
function drawClouds() {
...
''', None),
    ...
    ("feat: draw pipes — gradient body, rounded caps, shine, moving pipe indicator",
     '''
function drawPipe(p) {
...
''', None),
    ...
    ("feat: draw coins — spinning radial gradient with glint and draw power-up items",
     '''
function drawCoins() {
  for (const c of coins) {
...
''', None),
    ...
    ("feat: draw bird — invincibility blink, shield glow ring, magnet aura, crown",
     '''
function drawBird() {
...
''', None),
    ...
    ("feat: draw particles with glow and boundary dashed lines",
     '''
...
''', None),
    ...
    ("feat: idle bird floating animation for start and game-over screens",
     '''
// ── Idle Bird ──────────────────────────────────────────────
...
''', None),
    ...
    ("feat: main render loop — screen shake transform, full draw pipeline",
     '''
// ── Main Loop ──────────────────────────────────────────────
function loop() {
  frameCount++;
...
''', None),
    ...
    ("feat: bootstrap — init HUD, start screen, leaderboard, kick off game loop",
     '''
// ── Bootstrap ──────────────────────────────────────────────
...
# ── Build incrementally ────────────────────────────────────────────────────
print(f"\n🚀 Building 34 commits for Flappy Fahh Ultra Edition...\n")

accumulated_js = ""

for i, (msg, js_block, html_override) in enumerate(BLOCKS, 1):
    print(f"[{i:02d}/34] {msg[:70]}...")

    accumulated_js += js_block

    # Write the accumulated game.js so far
    write("game.js", accumulated_js)

    # For index.html: use the final version only on last commit, else use original
    if html_override is not None:
        write("index.html", html_override)

    commit(msg)

print("\n🔄 Pushing all 34 commits to GitHub...")
result = subprocess.run(["git", "push", "origin", "main"], cwd=DIR, capture_output=True, text=True)
if result.returncode == 0:
    print("✅ Pushed successfully!")
    print("🌐 https://github.com/Suraj-kummar/Flappy-Fahh-game")
else:
    print("❌ Push failed:", result.stderr)

# ── Restore final game.js ──────────────────────────────────────────────────
write("game.js", FINAL_GAME_JS)
write("index.html", FINAL_HTML)
git("add", "game.js", "index.html")
r = subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=DIR)
if r.returncode != 0:
    subprocess.run(["git", "commit", "-m", "fix: restore final complete game.js and index.html"], cwd=DIR)
    subprocess.run(["git", "push", "origin", "main"], cwd=DIR, capture_output=True)
    print("✅ Final files restored and pushed.")
